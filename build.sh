...
# Common source files and compiler flags
COMMON_SRC="src/common/common.c"
COMMON_FLAGS="-O2 -Wall -Isrc"

echo "[1/5] Building Classic DLL Injection..."
x86_64-w64-mingw32-gcc $COMMON_FLAGS -o bin/01_classic_dll_injection.exe src/01_classic_dll_injection/injector.c $COMMON_SRC
if [ $? -ne 0 ]; then
...
x86_64-w64-mingw32-gcc $COMMON_FLAGS -o bin/02_process_hollowing.exe src/02_process_hollowing/hollowing.c $COMMON_SRC
if [ $? -ne 0 ]; then
...
x86_64-w64-mingw32-gcc $COMMON_FLAGS -o bin/03_apc_injection.exe src/03_apc_injection/apc_injector.c $COMMON_SRC
if [ $? -ne 0 ]; then
...
x86_64-w64-mingw32-gcc $COMMON_FLAGS -o bin/04_thread_hijacking.exe src/04_thread_hijacking/hijack_thread.c $COMMON_SRC
if [ $? -ne 0 ]; then
...
x86_64-w64-mingw32-gcc $COMMON_FLAGS -o bin/05_reflective_dll_injection.exe src/05_reflective_dll_injection/reflective_loader_fixed.c $COMMON_SRC
if [ $? -ne 0 ]; then
...
x86_64-w64-mingw32-gcc -shared -o bin/sample_dll.dll src/sample_dll/sample_dll.c -luser32
if [ $? -ne 0 ]; then
